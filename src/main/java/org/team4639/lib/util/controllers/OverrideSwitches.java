...
package org.team4639.lib.util.controllers;
...
public class OverrideSwitches {
    ...
    public boolean getDriverSwitch(int index) {
        if (index < 0 || index > 2) {
            throw new RuntimeException("Invalid driver override index " + Integer.toString(index) + ". Must be 0-2.");
        }
    ...
    public boolean getOperatorSwitch(int index) {
        if (index < 0 || index > 4) {
            throw new RuntimeException("Invalid operator override index " + Integer.toString(index) + ". Must be 0-4.");
        }
